...
#define T 3
/* max length for the movie length */
#define MAXLEN 1024
...
int find_index(nodekey key, p_tnode pnode) {
	...
	while (L <= R) {
        M = (L + R) >> 1;
        icmp = strcasecmp(key, pnode->keys[M]);
        if (icmp > 0) {
            L = M + 1;
            ibetween = L;
        }
...
int main(int argc, char * argv[]) {
    ...
    /* buffer for movie name */
    char input[MAXLEN];

    printf("Please input the movie name: ");
    while (fgets(input, sizeof(input), stdin) != NULL) {
        ...
        if (input[0] == '\0') {
            printf("Input is an empty string, exiting...\n");
            exit(0);
        }
        
        nodevalue *result = find_value(input);
        if (result != NULL)
            display_record(result, stdout);
        else
            printf("%s not found\n", input);
        printf("Please input the movie name: ");
    }
